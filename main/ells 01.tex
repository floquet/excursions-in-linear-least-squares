\documentclass[10pt]{newsiambook}

\usepackage{amsmath,amssymb}
%\usepackage{epstopdf} 
\usepackage{color}
\usepackage{xcolor}
%\usepackage{transparent}
\usepackage{colortbl}
\setlength\minrowclearance{4pt}
%\usepackage{graphicx}
%\usepackage{pifont}
\usepackage{makeidx}
%\usepackage{multicol}
\usepackage{multirow}
\usepackage{crop}
\usepackage{pdfpages}
\usepackage{lscape}
%\usepackage{pdflscape}  %https://tex.stackexchange.com/questions/337/how-to-change-certain-pages-into-landscape-portrait-mode
\usepackage{rotating}
%\usepackage{tabularx}
%\usepackage{pstricks}
%\usepackage{sidecap}
\usepackage{stmaryrd}  % https://tex.stackexchange.com/questions/26508/left-version-of-mapsto
%\usepackage{epstopdf}
\usepackage{siunitx}
%https://tex.stackexchange.com/questions/7735/how-to-get-straight-quotation-marks
%\usepackage[T1]{fontenc}
%\usepackage{upquote}
\usepackage{textcomp}  % http://hevea.inria.fr/examples/test/sym.html
\usepackage{tikz}
\usepackage{overpic}
\usepackage[nomessages]{fp} % http://tex.stackexchange.com/questions/30081/how-can-i-sum-two-values-and-store-the-result-in-other-variable
 
\crop	
\newcommand{\pathname}{../"common"/}
\newcommand{\fullpath}{\pathname}
\newcommand{\pathgraphics}{../pdf/}
%\newcommand{\patheps}     {../eps/}

\input{\pathname declarations.tex}
	
\makeindex

\begin{document}

\frontmatter
%
\tableofcontents
%
\listoffigures
%%
\listoftables
%
%\input{chapters/frontmatter/preface}
%\input{chapters/frontmatter/literature}

\input{../frontmatter/"ch literature"}

\mainmatter
%%%%%%%
\part{Rudiments}
\input{../parts/01_rudiments/"ch least squares problems"}
\input{../parts/01_rudiments/"ch least squares theory"}

%%%%%%%
\part{Essential Examples}
\input{../parts/02_essentials/"ch mean"}
\input{../parts/02_essentials/"ch lines"}
%\input{../parts/rudiments/"ch grid"}

%%%%%%%
\part{Modal Example}
A sample problem examined from a few different perspectives.

\input{../parts/03_modal_archetypes/"ch modal calculus"}
\input{../parts/03_modal_archetypes/"ch modal svd"}
\input{../parts/03_modal_archetypes/"ch modal others"}
\input{../parts/03_modal_archetypes/"ch modal finer points"}

%%%%%%%
\part{Zonal Example}
\input{../parts/04_zonal_archetypes/"ch zonal"}

%%%%%%%
%\part{Least Squares Theory}

%%%%%%%
%\part{Applications: Polynomial Approximation}

%%%%%%%
%\part{Applications:\\Finding Patterns}
\input{../parts/patterns/"ch patterns lines"}
\input{../parts/patterns/"ch patterns crystals"}

%%%%%%%
%\part{Applications: Stitching}
\input{../parts/stitch/"ch stitch piston"}

%%%%%%%
%\part{Applications:\\Inverting the Gradient}
\input{../parts/gradient/"ch gradient"}

%%%%%%%
\part{Applications:\\Nonlinear Problems}
Can I write something here?
\input{../parts/nonlinear/"ch best circle"}
\input{../parts/nonlinear/"ch linearization"}
\input{../parts/nonlinear/"ch census"}


%%%%%%%%
\part{Appendices}
\appendix
\input{../appendices/"app exercises"}
%\input{../appendices/"app exemplars"}
%\input{../appendices/"app errors"}
%\input{../appendices/"app notation"}
%\input{../appendices/"app jargon"}

%%%%%%
\part{\ \ Backmatter}
\backmatter
%\input{backmatter/acronyms}
%\input{chapters/backmatter/notation/"head notation"}
%%\input{backmatter/glossary}
%\bibliography{chapters/backmatter/bibliography/svdfp}
%\input{chapters/backmatter/bibliography/svdfp.bib}

\input{../"ells bibliography"}

\printindex

\end{document}  %   .   .   .   .   .   .   .   .   .   .   .   .   .   .   .   .   .   .

%  \tiny
%  \scriptsize
%  \footnotesize
%  \small
%  \normalsize
%  \large
%  \Large
%  \LARGE
%  \huge
%  \Huge

% \section{sec} %    S    S    S    S    S    S    S    S    S    S    S    S
% \subsection{ssec}  %   SS   SS   SS   SS   SS   SS   SS   SS   SS   SS   SS   SS
